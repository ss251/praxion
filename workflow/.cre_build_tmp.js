...
var addr = (s) => s;
var REPORT_PARAMS = parseAbiParameters("bytes32 reportId, address vault, address agent, uint8 verdict, (address sellToken, address buyToken, uint256 sellAmount, uint256 minBuyAmount, uint256 deadline) intent, uint256 navUsd6, uint16 postTradeExposureBps, uint256 expectedPriceUsd6, uint16 slippageBps, string reason");
...
var buildPriceRequest = (url) => (sendRequester, _config) => {
...
function onHttpTrigger(runtime2, payload) {
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  runtime2.log("Praxion v2: Trade Evaluation Workflow");
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  try {
    if (!payload.input || payload.input.length === 0) {
      throw new Error("Empty request payload");
    }
    const proposal = decodeJson(payload.input);
    runtime2.log(`[Step 1] Trade proposal from agent ${proposal.agent}`);
    runtime2.log(`[Step 1] ${proposal.sellToken} → ${proposal.buyToken}, amount: ${proposal.sellAmount}`);
    runtime2.log("[Step 2] Reading on-chain state...");
    ...
    const policyResult = evmClient.callContract(runtime2, {
    ...
    runtime2.log(`[Step 2a] Policy: maxTrade=${constraints.maxTradeNotionalUsd6}, maxSlippage=${constraints.maxSlippageBps}bps, maxPosition=${constraints.maxPositionBps}bps, cooldown=${constraints.cooldownSeconds}s`);
    const agentActiveData = encodeFunctionData({
      abi: REGISTRY_ABI,
      functionName: "isActiveAgent",
      args: [proposal.agent]
    });
    const agentActiveResult = evmClient.callContract(runtime2, {
    ...
    runtime2.log(`[Step 2b] Agent active: ${isActive}`);
    ...
    runtime2.log(`[Step 2c] Agent stake: ${agentStake}`);
    const lastTradeData = encodeFunctionData({
      abi: VAULT_ABI,
      functionName: "lastTradeTime",
      args: [proposal.agent]
    });
    const lastTradeResult = evmClient.callContract(runtime2, {
    ...
    runtime2.log(`[Step 2d] Last trade time: ${lastTradeTime}`);
    let assetAllowed = true;
    if (constraints.onlyAllowedAssets) {
      const allowedData = encodeFunctionData({
        abi: POLICY_ABI,
        functionName: "isAllowedAsset",
        args: [proposal.vault, proposal.buyToken]
      });
      const allowedResult = evmClient.callContract(runtime2, {
    ...
    runtime2.log(`[Step 2e] Asset allowed: ${assetAllowed}`);
    const usdcBalData = encodeFunctionData({
      abi: ERC20_ABI,
      functionName: "balanceOf",
      args: [proposal.vault]
    });
    const usdcBalResult = evmClient.callContract(runtime2, {
    ...
    runtime2.log(`[Step 2f] Vault USDC balance: ${vaultUsdcBalance}`);
    runtime2.log("[Step 3] Fetching prices from 3 sources...");
    const chainlinkCallData = encodeFunctionData({
      abi: AGGREGATOR_V3_ABI,
      functionName: "latestRoundData",
      args: []
    });
    const chainlinkResult = evmClient.callContract(runtime2, {
    ...
    runtime2.log(`[Step 3a] Chainlink on-chain ETH/USD: $${chainlinkPriceUsd.toFixed(2)} (raw: ${rawAnswer})`);
    const httpClient = new cre.capabilities.HTTPClient;
    const price1 = httpClient.sendRequest(runtime2, buildPriceRequest(runtime2.config.priceFeedUrl1), consensusIdenticalAggregation())(runtime2.config).result();
    runtime2.log(`[Step 3b] CoinGecko ETH/USD: $${price1.priceUsd}`);
    let price2 = { priceUsd: 0 };
    let hasCoinPaprika = false;
    try {
      price2 = httpClient.sendRequest(runtime2, buildPriceRequest(runtime2.config.priceFeedUrl2), consensusIdenticalAggregation())(runtime2.config).result();
      hasCoinPaprika = price2.priceUsd > 0;
      runtime2.log(`[Step 3c] CoinPaprika ETH/USD: $${price2.priceUsd}`);
    } catch (e) {
      runtime2.log(`[Step 3c] CoinPaprika unavailable (timeout), using 2-source consensus`);
    }
    const prices = hasCoinPaprika ? [chainlinkPriceUsd, price1.priceUsd, price2.priceUsd].sort((a, b) => a - b) : [chainlinkPriceUsd, price1.priceUsd].sort((a, b) => a - b);
    const medianPrice = prices.length === 3 ? prices[1] : (prices[0] + prices[1]) / 2;
    const expectedPriceUsd6 = BigInt(Math.round(medianPrice * 1e6));
    const maxPrice = Math.max(...prices);
    const minPrice = Math.min(...prices);
    const sourceDivergenceBps = medianPrice > 0 ? Math.round((maxPrice - minPrice) / medianPrice * 1e4) : 0;
    runtime2.log(`[Step 3d] Consensus price (${prices.length} sources): $${medianPrice.toFixed(2)} (${expectedPriceUsd6} USD6)`);
    runtime2.log(`[Step 3d] Source divergence: ${sourceDivergenceBps} bps`);
    runtime2.log("[Step 4] Evaluating constraints...");
    const sellAmount = BigInt(proposal.sellAmount);
    const minBuyAmount = BigInt(proposal.minBuyAmount);
    const tradeNotionalUsd6 = sellAmount;
    const expectedBuyAmount = sellAmount * BigInt(1000000000000000000) / expectedPriceUsd6;
    const navUsd6 = vaultUsdcBalance;
    const postTradeExposureBps = navUsd6 > 0n ? Number(tradeNotionalUsd6 * 10000n / navUsd6) : 1e4;
    const slippageBps = expectedBuyAmount > 0n ? Number((expectedBuyAmount - minBuyAmount) * 10000n / expectedBuyAmount) : 0;
    const currentTime = BigInt(Math.floor(Date.now() / 1000));
    const cooldownElapsed = lastTradeTime === 0n || currentTime - lastTradeTime >= BigInt(constraints.cooldownSeconds);
    const reasons = [];
    if (!isActive) {
    ...
    runtime2.log(`[Step 4] Verdict: ${approved ? "APPROVE ✓" : "REJECT ✗"}`);
    runtime2.log(`[Step 4] Reason: ${reasonStr}`);
    runtime2.log(`[Step 4] Post-trade exposure: ${postTradeExposureBps}bps`);
    runtime2.log(`[Step 4] Slippage: ${slippageBps}bps`);
    runtime2.log("[Step 5] Writing verdict report to PraxionSettlement...");
    const reportId = keccak256(toHex(`${proposal.agent}-${proposal.vault}-${proposal.sellAmount}-${proposal.deadline}-${currentTime}`));
    ...
    if (writeResult.txStatus === TxStatus.SUCCESS) {
      ...
      runtime2.log(`[Step 5] ✓ Report written: ${txHash}`);
      runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
...
var addr2 = (s) => s;
var REPORT_STORED_ABI = parseAbi([
...
function onReportStoredLog(runtime2, log) {
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  runtime2.log("Praxion: Report Monitor — Post-Verdict Analysis");
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  try {
    const topics = log.topics.map((t) => bytesToHex(t));
    const data = bytesToHex(log.data);
    const decoded = decodeEventLog({ abi: REPORT_STORED_ABI, data, topics });
    const reportId = decoded.args.reportId;
    const verdict = Number(decoded.args.verdict);
    const vault = decoded.args.vault;
    const agent = decoded.args.agent;
    const verdictStr = verdict === 0 ? "APPROVE ✓" : "REJECT ✗";
    runtime2.log(`[Step 1] Report ${reportId.slice(0, 10)}... — ${verdictStr}`);
    runtime2.log(`[Step 1] Vault: ${vault}`);
    runtime2.log(`[Step 1] Agent: ${agent}`);
    runtime2.log("[Step 2] Reading full report from PraxionSettlement...");
    const evmConfig = runtime2.config.evms[0];
    const network248 = getNetwork({
      chainFamily: "evm",
      chainSelectorName: evmConfig.chainSelectorName,
    ...
    const readResult = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: addr2(evmConfig.settlementAddress),
        data: callData
      })
    }).result();
    const report2 = decodeFunctionResult({
      abi: GET_REPORT_ABI,
      functionName: "getReport",
      data: bytesToHex(readResult.data)
    });
    runtime2.log("[Step 3] Full Report Analysis:");
    runtime2.log(`  NAV (USD6):           ${report2.navUsd6}`);
    runtime2.log(`  Post-Trade Exposure:  ${report2.postTradeExposureBps} bps`);
    runtime2.log(`  Expected Price:       ${report2.expectedPriceUsd6} USD6`);
    runtime2.log(`  Slippage:             ${report2.slippageBps} bps`);
    runtime2.log(`  Reason:               ${report2.reason}`);
    runtime2.log(`  Sell:                 ${report2.intent.sellToken}`);
    runtime2.log(`  Buy:                  ${report2.intent.buyToken}`);
    runtime2.log(`  Sell Amount:          ${report2.intent.sellAmount}`);
    if (verdict === 1) {
      runtime2.log("[ALERT] ⚠️  REJECT verdict — agent may be slashed");
    } else {
      runtime2.log("[OK] ✓ Trade approved — agent can proceed with execution");
    }
    runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    return JSON.stringify({
      reportId,
      verdict: verdictStr,
      navUsd6: report2.navUsd6?.toString() || "0",
      exposureBps: report2.postTradeExposureBps,
      slippageBps: report2.slippageBps,
...
var REPORT_STORED_SIGNATURE = "ReportStored(bytes32,uint8,address,address)";
